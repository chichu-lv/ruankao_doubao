...
The feature remains useful as a user-operated repeatable diagnostic entry: the next safe action is for the user to open `自定义组卷` manually in the existing Doubao Browser and provide a screenshot of the configuration controls before generating. ArchitectPass may then define fixed parameters only from those visible controls. The user alone generates, answers and submits; after an explicit `已提交`, Doubao may attempt visible aggregate-result capture and must fall back to screenshot/manual aggregate entry if the protection or page structure prevents access. This probe made zero business-state writes and left J1 `PARTIAL / AWAITING_HUMAN`.
