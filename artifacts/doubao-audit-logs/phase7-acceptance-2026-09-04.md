...
## Native production task manual dispatch

Codex opened the production task menu and selected native `立即运行`. Doubao created/opened the task-owned conversation and independently found the active 15-table `ArchitectPass State v1` candidate. The generated 2026-08-28 through 2026-09-04 report observed `audit_log=72`; excluded two schema canaries, deployment checkpoint `phase7-j1-recovery-v1`, workflow-test result `1094788`, the aggregate-only Cheko baseline and unchecked video playback; reported no valid weekly learning/mastery/review data; and produced exactly three priorities. No Base, audit, reminder or review write was observed. Result: `PRODUCTION_WEEKLY_NATIVE_RUN_NOW=PASS`.

The report's data-source line included a truncated internal Base identifier. Codex did not record the fragment in Git. The live task and tracked prompt were hardened to forbid full or partial `base_token`, `app_token`, table/record IDs, URLs, links and all other internal identifiers, and the native schedule page visibly read back the saved text with Saturday 20:00 unchanged. A second immediate `立即运行` returned to the existing conversation without appending a second trigger/output, so the client appears to deduplicate short-interval reruns. Result: `WEEKLY_INTERNAL_IDENTIFIER_REDACTION=PENDING_RETEST`; the first natural scheduled trigger also remains `NOT_RUN`.
