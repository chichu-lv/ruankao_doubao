...
Status: `NOT_RELEASED / WAITING_FOR_AUTHORIZATION`. Recipient OS is Windows. Main fixes are pushed through `50202f7` (1.1.1); 94 local tests and the 19-check accelerated pilot pass. All 349 authorized files are present; all 209 PDFs (8,128 pages) open and render their first page, and all 121 MP4s expose video tracks and positive durations. Real Doubao installed from an independently extracted package on Mac, indexed two PDFs into 44 pages, created a separate 15-table Feishu Base, and verified a 1-profile/10-topic/2-resource/13-audit initialization with zero-write replay.

The real exercise caught and corrected duplicate topic creation caused by CLI response misparsing; a new live-write protocol and late-bound Base configuration are in main. Cheko's observed anti-debug page is an explicit manual-result fallback, not an automation PASS. Windows Actions results cannot yet be read; requested test-log branch write permission was blocked by automatic review and remains uncommitted pending explicit authorization. Doubao's real project creation is also paused at its computer-control system authorization, so a local project-name file is not counted as a created project. No final Windows ZIP or one-prompt full-install PASS is claimed. Details: `docs/test-results/final-delivery-windows-2026-09-05.md`.
