...
Phase 7 should continue from revised Step D of the real 90-minute plan. The target date, optional user-provided score history, preference to skip exam-background material and Doubao Browser Cheko handoff preference are recorded; the seven-section blanket self-rating remains conversation-only and Step B is PARTIAL because recall boundaries were unknown. After the user authorized computer operation, direct ordinary UI control reached the real custom-paper form and saved `AP-J1-计算机网络基线-v1`: 20 questions, balanced 6/10/4 difficulty, unattempted-first, computer-network only, second-edition plus must-master labels, and five recent periods excluding the user's two reported exam sittings. The browser is stopped before `出题`; the user alone must start, answer and submit, and Doubao may resume only after the user's `已提交` signal to read visible aggregate results. The session must finish with the first checkpoint.
...
Direct ordinary UI operation in the existing Doubao Browser later reached the form without bypassing protection, verified all available filter groups and saved `AP-J1-计算机网络基线-v1`; the enabled `出题` action was deliberately not used. The second authorized Baidu material root was also enumerated: eight PDFs are present, including explicit choice and case `真题分类解析` titles. These entries are inventory-only until downloaded, hashed and content-verified. Phase 7 now waits for the user to run and submit the saved paper.
