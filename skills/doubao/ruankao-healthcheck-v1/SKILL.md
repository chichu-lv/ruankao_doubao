...
健康记录的 `checked_at` 直接采用本次快照生成的带时区时间，不得手写或从示例复制。报告较晚完成时重新运行快照取得实际时刻；UTC 转成本地时间必须转换时刻，不能仅把 `+00:00` 换成 `+08:00`。学习事件与审计时间同样按写入协议取真实系统时钟。
...
5. Git/网盘安装时检查豆包浏览器、百度网盘人工定位路径；完整离线包已有资料时，此网盘登录项标为不适用，不因未登录网盘降低离线安装状态。
   公开源码安装但授权资料已在本机可读时，也不检索/调用网盘来补健康项；远端记 `DEFERRED_NOT_REQUIRED_FOR_LOCAL_STUDY`，本地材料能力按索引事实单独评分。只有缺少具体学习文件才启动远端获取。
