...
短时学习优先选能实际读取并定位的小单元。一份扫描 PDF 不可读时，先在本次清单内寻找可读 PDF（按需增量处理该文件/必要页），再考虑视频。视频文件名和总时长不能证明前几分钟讲了什么；没有字幕、转写或实际片段观察时，不声称某时间段覆盖指定考点，标为“内容时间点待核验”，让用户正常观看定位或换可核验资料。没有播放回读或用户陈述，不写已观看进度。
