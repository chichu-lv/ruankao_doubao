...
```text
你就是当前执行部署的豆包，不是 Codex。不要打开另一个豆包或在虚拟桌面登录豆包。先确认工具操作我这台电脑；无法确认时停止安装，指导我在桌面客户端新建“本地电脑”任务。请使用本机已解压的 ArchitectPass-offline 目录部署“架构上岸教练”。先读取 project/deployment/doubao/execution-context-v1.md、README-OFFLINE.md，再执行 project/deployment/offline/bootstrap-v1.md。自身项目或技能界面无法操作时，只让我完成具体点击，不转用网页豆包。用包内代码、资料和九个技能完成部署；已有本次项目和状态先核对后继续，只有首次无档案时才建立独立空档案。连接我自己的飞书和芝士架构账号，完成真实健康检查后开始训练。我授权本次安装所需的项目内操作；第三方登录和系统授权由我本人完成，不重做 Codex 开发或能力审计。
```
