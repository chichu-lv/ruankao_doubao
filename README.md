...
```text
请从公开仓库 https://github.com/chichu-lv/ruankao_doubao 的 main 部署“架构上岸教练”，按 README.md 和 deployment/doubao/bootstrap-v1.md 执行。我只会使用豆包，请使用本地电脑自动下载源码 ZIP、准备环境和部署，不要求我安装 Git、配置 SSH 或手输命令。Windows 百度网盘客户端已安装并登录，请先核对其中两个授权课程目录，按需下载索引。为我建立独立的空学习档案，连接我自己的飞书和芝士架构，完成健康检查后开始训练。安装所需项目内操作已授权；登录和系统授权需要我操作时，请打开官方入口并逐步指引。不要继承资料提供者的学习进度，不修改其它项目。
```

用户只需：安装并登录豆包桌面客户端 → 新工作任务选择「本地电脑」→ 发送上述提示词 → 按引导完成本人登录/授权。豆包应说明当前正在准备什么、卡在哪里，不能把系统权限、验证码或账号授权假装成已自动批准。Windows 授权界面的逐屏引导仍以当前实际界面为准。
