...
运行前确保用户已在豆包完成所需“操作电脑”系统授权。新工作任务页的「项目 → 创建新项目」是已实测入口。若桌面辅助功能仅能读取菜单栏，可以使用同账号的豆包官方网页 `https://www.doubao.com/chat` 创建项目，并在桌面侧边栏验证同步；不读写客户端内部存储来伪造项目。

网页新任务可能默认「云电脑」，不能读取本机离线包。项目创建后，回到桌面客户端并确认任务选择「本地电脑」，再读取代码、资料及当前 Base 绑定。没有持久指令字段时，把系统指令和部署配置作为该项目的私有上下文文件，新增同项目对话后实际回读验证，不能仅靠安装对话的记忆。
