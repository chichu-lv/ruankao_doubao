...
5. 先读 `project/deployment/doubao/folder-skills-v1.md`，按当前官方「创建技能」指南解析本机 `workspace/.user_skills`，由豆包运行文件夹安装器一次安装九个技能及本机位置绑定。文件存在不等于发现通过，必须在新本地任务验证调用和状态恢复。仅在官方方式不可用时才请用户导入 `prebuilt-skills/` 的 ZIP；只复用同内容且同绑定的技能。侧边栏项目与本机工作区分别报告：需要侧边栏项目时创建 `架构上岸教练` 并绑定实际 project 文件夹；无关同名项目保留。当前 Mac 没有项目指令/单独上下文字段，不要求用户寻找；系统指令经技能安装绑定读取。不转用虚拟桌面。
6. 使用当前账号的飞书连接器，在用户选择的私人空间创建 `ArchitectPass State v1`，按 `schemas/feishu-bitable-v1.json` 建立 15 张表。已有 Base 时先确认是本次要继续使用的学习状态。测试安装使用另名 Base。把实际项目名称、Base 名称与标识、资料路径保存在 `dist/deployment/project-state.json`，各技能通过安装绑定读取此文件；后续始终使用该绑定。操作前必须阅读 `deployment/feishu/write-protocol-v1.md`，不能把回读解析失败当作零记录后重复新增。
